...
/// <summary>
/// Executes a <c>dev-start-dotnet new</c> scaffold: resolves capability
...
public sealed class Planner
{
    ...
    /// + git init should use <see cref="RunAsync"/>. <c>dev-start-dotnet upgrade</c>
    ...
    private static void TryGitInit(string target)
    {
        try
        {
            ...
            Run("git", "-c user.name=dev-start-dotnet -c user.email=noreply@dev-start-dotnet.dev commit -m \"chore: initial scaffold by dev-start-dotnet\" --no-gpg-sign --quiet", target);
        }
        catch
        {
