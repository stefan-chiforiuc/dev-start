...
The .NET global tool that drives `dev-start-dotnet`. Published to NuGet as
[`DevStartDotnet`](https://www.nuget.org/packages/DevStartDotnet).
...
```sh
dotnet build src/DevStartDotnet.Cli
```

## Pack

```sh
dotnet pack src/DevStartDotnet.Cli -c Release -o artifacts
```
...
dev-start-dotnet --version       # confirm
dev-start-dotnet --help
```
...
src/DevStartDotnet.Cli/
  ...
  Baselines.cs              # .dev-start-dotnet/baselines.json for upgrade --apply
