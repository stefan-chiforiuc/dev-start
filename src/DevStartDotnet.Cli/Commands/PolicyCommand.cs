...
/// <summary>
/// <c>dev-start-dotnet policy</c> — list / apply / remove / validate org-level
...
public static class PolicyCommand
{
    ...
    private static Command BuildApply()
    {
        ...
        apply.SetHandler((name, projectPath) =>
        {
            ...
            {
                AnsiConsole.MarkupLine($"[red]Unknown policy[/] '{name}'. Run [cyan]dev-start-dotnet policy list[/].");
                return;
            }
            ...
            AnsiConsole.MarkupLine($"[bold]dev-start-dotnet policy apply[/] [cyan]{name}[/]");
