...
/// <summary>
/// Scaffolding helpers for capability authors. <c>dev-start-dotnet capability new foo</c>
...
public static class CapabilityCommand
{
    ...
    private static Command BuildNew()
    {
        ...
        var rootOpt = new Option<string>(["--root", "-r"], () => ".",
            "Root of the dev-start-dotnet checkout (defaults to cwd).");
        ...
        cmd.SetHandler((name, repoRoot) =>
        {
            ...
            {
                AnsiConsole.MarkupLine($"[red]Skeleton not found[/]: {skeleton}. Run this from a dev-start-dotnet checkout.");
                return;
            }
