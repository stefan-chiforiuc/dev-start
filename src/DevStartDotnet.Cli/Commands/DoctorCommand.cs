...
namespace DevStartDotnet.Commands;
...
public static class DoctorCommand
{
    public static Command Build()
    {
        ...
        cmd.SetHandler(async (projectPath, fix) =>
        {
            ...
            AnsiConsole.MarkupLine($"[bold]dev-start-dotnet doctor[/] [grey]→[/] {root}");
            ...
            table.AddRow("project", ".dev-start-dotnet.json", File.Exists(Path.Join(root, ".dev-start-dotnet.json"))
                ? "[green]ok[/]"
                : "[red]missing[/]");
            ...
            else if (missingEnv.Count > 0)
            {
                ...
                AnsiConsole.MarkupLine(
                    $"[grey]{missingEnv.Count} missing env key(s) can be auto-populated with[/] [cyan]dev-start-dotnet doctor --fix[/].");
            }
        }, projectOpt, fixOpt);
    ...
    private static void ApplyFixes(string projectRoot, IReadOnlyList<Capability.EnvAddition> missing)
    {
        ...
        if (!existing.Contains("# dev-start-dotnet doctor --fix", StringComparison.Ordinal))
        {
            writer.WriteLine("# dev-start-dotnet doctor --fix — placeholder values from capability examples");
