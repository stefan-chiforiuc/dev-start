...
/// <c>dev-start-dotnet new</c>, <c>dev-start-dotnet add</c>, or an applied upgrade.
/// Used by <c>dev-start-dotnet upgrade --apply</c> to distinguish
/// user-edited files (disk != baseline) from template-refresh-only
/// files (disk == baseline).
///
/// Stored at <c>.dev-start-dotnet/baselines.json</c> — separate from the user-facing
/// <c>.dev-start-dotnet.json</c> so diffs of the manifest stay human-readable.
/// Committed to the repo so baselines travel with clones.
/// </summary>
public sealed class Baselines
{
    private const string RelativePath = ".dev-start-dotnet/baselines.json";
